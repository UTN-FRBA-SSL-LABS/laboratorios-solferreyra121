...
int carrito_agregar(Carrito *c, Producto p) {
    if (c->cantidad < MAX_ITEMS) {   /* BUG: deberia ser < MAX_ITEMS */ /* ya se corrigio*/
...
int carrito_total(Carrito *c) {
    ...
    for (int i = 0; i < c->cantidad; i++) {
        total += c->items[i].precio * c->items[i].cantidad;  /* BUG: falta multiplicar por cantidad */ /* ya se agrego*/
    }
    return total;
}
